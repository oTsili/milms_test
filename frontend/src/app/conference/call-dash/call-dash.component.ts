...
import { SignalingService, SignalMessage } from './../signaling.service';
import {
  Component,
  OnInit,
  ViewChild,
...
export class CallDashComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.roomName = this.actRt.snapshot.queryParams['roomName'] || 'room_users';
        ...
        this.signalingService.onConnect(() => {
          ...
          this.signalingService.onRoomParticipants((participants) => {
            ...
            this.signalingService.sendOfferSignal({
              signalData: { type: 'offer', sdp: 'kldjfdfkgjdkjk' },
              callerId: this.signalingService.socketId,
              calleeId: participants.find(
          ...
          this.signalingService.onRoomLeft((socketId) => {
            this.mitronPeers = this.mitronPeers.filter(
